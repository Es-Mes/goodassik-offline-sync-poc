/**
 * Integration Test Helpers - Fixed for ExamScans structure
 * Utilities for testing full sync workflow with Docker services
 */
...
const localApi = {
    ...
    createScan: async (studentId, examId) => {
        ...
        const imagePath = localApi.createTestImage();
        formData.append('image', fs.createReadStream(imagePath), {
            filename: 'test-scan.jpg',
            contentType: 'image/jpeg'
        });

        const response = await axios.post(`${LOCAL_API}/api/scans`, formData, {
    ...
    getScan: async (scanId) => {
        const response = await axios.get(`${LOCAL_API}/api/scans/${scanId}`);
        return response.data.scan;
    },

    updateScan: async (scanId, updates) => {
        const response = await axios.patch(`${LOCAL_API}/api/scans/${scanId}`, updates);
        return response.data.scan;
    },
...
const cloudApi = {
    getScan: async (cloudScanId) => {
        const response = await axios.get(`${CLOUD_API}/api/scans/${cloudScanId}`);
        return response.data.scan;
    },

    updateScan: async (cloudScanId, updates) => {
        const response = await axios.patch(`${CLOUD_API}/api/scans/${cloudScanId}`, updates);
        return response.data.scan;
    },

    getUpdates: async (limit = 100) => {
        const response = await axios.get(`${CLOUD_API}/api/scans/updates?limit=${limit}`);
        return response.data;
    },

    markUpdateComplete: async (updateId) => {
        const response = await axios.post(`${CLOUD_API}/api/scans/updates/${updateId}/complete`);
        return response.data;
    }
};
...
const dbHelpers = {
    ...
    getCloudOutboxStatus: async (cloudScanId) => {
        const client = await getCloudDb();
        try {
            const result = await client.query('SELECT * FROM cloudsyncoutbox WHERE entityid = $1', [cloudScanId]);
            return result.rows.length > 0 ? result.rows[0] : null;
        } finally {
            await client.end();
        }
